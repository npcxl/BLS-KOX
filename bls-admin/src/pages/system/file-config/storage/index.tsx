  ...
  const formColumns: ProFormColumnsType<StorageRecord>[] = [
    {
      ...
      formItemProps: { rules: [{ required: true, message: "请输入存储名称" }] },
    },
    {
      title: "存储类型",
      dataIndex: "storageType",
    ...
    },
    {
      ...
      dataIndex: "publicBucket",
      formItemProps: {
        rules: [{ required: true, message: "请输入公共桶名称" }],
      },
    },
    ...
    },
    {
      ...
      transform: (value) => toJsonString(value),
    },
    {
      title: "策略 JSON",
      dataIndex: "policyJson",
      valueType: "textarea",
      fieldProps: {
        autoSize: { minRows: 4, maxRows: 10 },
        style: { fontFamily: "monospace" },
  ...
  return (
    <CrudTablePage<StorageRecord>
      ...
      beforeSubmit={(values, current) => ({
        ...
        isDefault: String(values.isDefault ?? current?.isDefault ?? "0") as
          | "0"
          | "1",
        status: String(values.status ?? current?.status ?? "1") as "0" | "1",
        configJson: toJsonString(
          values.configJson || current?.configJson || ""
        ),
        policyJson: toJsonString(
          values.policyJson || current?.policyJson || ""
        ),
      })}
    />
  );
