...
| 6 | `/api/auth/captcha/config` | GET | `ip` | 120 | 60 s |
| 7 | `/api/auth/captcha/challenge` (layer 1) | GET | `ip` | 60 | 60 s |
| 8 | `/api/auth/captcha/challenge` (layer 1) | GET | `device` | 30 | 300 s |
| 9 | `/api/auth/captcha/verify` (layer 1) | POST | `ip` | 30 | 60 s |
| 10 | `/api/auth/captcha/verify` (layer 1) | POST | `account` | 20 | 300 s |
| 11 | `/api/auth/captcha/secondary/challenge` (layer 2) | POST | `ip` | 20 | 60 s |
| 12 | `/api/auth/captcha/secondary/challenge` (layer 2) | POST | `account` | 10 | 300 s |
| 13 | `/api/auth/captcha/secondary/verify` (layer 2) | POST | `ip` | 20 | 60 s |
| 14 | `/api/auth/captcha/secondary/verify` (layer 2) | POST | `account` | 10 | 300 s |
| 15 | `/api/auth/captcha/secondary/verify` (layer 2) | POST | `device` | 30 | 300 s |
| 16 | `/api/**` | POST/PUT/PATCH/DELETE | `user` | 300 | 60 s |
| 17 | `/api/**` | GET/HEAD/OPTIONS | `user` | 600 | 60 s |
...
server CPU, and the layer-2 endpoints additionally consume the **external** Tianai service) and
Redis memory. Layer-2 limits are tighter for that reason.
...
| Captcha layer-1 challenge (`GET /api/auth/captcha/challenge`) | 60 / min / IP + 30 / 5 min / device |
| Captcha layer-1 verify (`POST /api/auth/captcha/verify`) | 30 / min / IP + 20 / 5 min / account |
| Captcha layer-2 (`POST /…/secondary/challenge` and `/…/secondary/verify`) | 20 / min / IP + 10 / 5 min / account (+ 30 / 5 min / device on verify) |
| Captcha public config (`GET /api/auth/captcha/config`) | 120 / min / IP |
