...
| `auth/captcha.ts` | `getCaptchaConfig`, `verifySilentCaptcha`, `createSecondaryChallenge`, `verifySecondaryCaptcha`, `CAPTCHA_ERROR_CODES`, `CAPTCHA_REASON_TEXT` + types | ✔ Live — two-level login captcha (`pages/user/login`, `hooks/useLoginCaptcha`). All calls pass `skipErrorMessage:true`; the login page/hook owns the error UX. The layer-1 challenge itself is fetched by the official widget; the layer-2 challenge is fetched by the hook. **`stage` is never sent** — only `username` / `payload` / `sessionId` / `data`. |
| `system/config.ts` | `batchUpdateConfigs` | ✔ Live — `POST /api/system/config/batch` (single transaction, `system:config:edit`). Used by the captcha panel so a multi-field change can never be applied partially; the backend health-checks Tianai before committing. |
| `components/AltchaCaptcha/index.tsx` | `AltchaCaptcha` (forwardRef) | ✔ Live — **layer 1 only**. Thin React wrapper around the **official** `<altcha-widget>`: mounts it with `challenge`/`auto`/`display`/`language="zh-cn"`, imports `altcha` + `altcha/i18n/zh-cn`, forwards `verified`/`statechange`/`expired`, and themes it through the official `--altcha-*` CSS variables. `display` accepts only `invisible \| standard` (`visible` is not a valid ALTCHA value) and the visible form is **not** treated as the second layer. No captcha logic lives here. |
| `components/TianaiCaptcha/index.tsx` | `TianaiCaptcha` | ✔ Live — **layer 2** renderer for the Tianai challenge (`blockPuzzle` slider / `clickWord` point picking). Renders images forwarded by the server, collects answers, submits `{sessionId, data}`; keyboard-operable (arrow keys + Enter) and shows a refresh action when image fields are missing. It never uses `altcha-widget` and never decides correctness. |
| `pages/user/login/captcha-machine.ts`, `hooks/useLoginCaptcha.ts` | pure reducer + hook | ✔ Live — explicit phases (`loadingConfig`/`waitingUsername`/`solvingSilent`/`secondaryRequired`/`solvingSecondary`/`ready`/`submitting`/`error`); blocks submission until the config is loaded, debounces the username 400 ms, invalidates stale results via `cycleId`, clears the token after every `/login` call. |
