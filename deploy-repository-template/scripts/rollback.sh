...
# rollback.sh — 回滚到指定版本
#
# 用法: ./scripts/rollback.sh 1.0.0  (指定版本)
...
log "拉取镜像..."
docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" pull bls-admin bls-server bls-ai-service 2>&1 | tee -a "$ROLLBACK_LOG"

log "重启服务..."
docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" up -d --remove-orphans bls-admin bls-server bls-ai-service 2>&1 | tee -a "$ROLLBACK_LOG"
