...
CREATE TABLE IF NOT EXISTS `ops_release_version` (
  ...
  `status` varchar(20) NOT NULL DEFAULT 'building' COMMENT 'building/built/failed/unavailable',
...
CREATE TABLE IF NOT EXISTS `ops_release_task` (
  ...
  `action` varchar(20) NOT NULL DEFAULT 'deploy' COMMENT 'deploy/rollback',
  `from_version` varchar(20) DEFAULT NULL,
  `target_version` varchar(20) NOT NULL,
  `services` text NOT NULL COMMENT '服务列表(逗号分隔)',
...
-- ========== 菜单（按 Init.sql 格式：menu_id 为十六进制序号） ==========
-- 运维管理目录 (sort_num=90, 排在现有菜单最后)
...
-- ========== 字典数据（先删后插保证幂等） ==========
DELETE FROM `sys_dict_data` WHERE `dict_type_id` = 'ops_release_status';
DELETE FROM `sys_dict_type` WHERE `dict_type_id` = 'ops_release_status_type';

INSERT INTO `sys_dict_type` (`dict_type_id`, `dict_name`, `dict_type`, `status`, `remark`, `tenant_id`, `deleted`) VALUES
('ops_release_status_type', '发布任务状态', 'ops_release_status', '0', '发布中心任务状态字典', '000000', 0);
...
('dict_ops_rel_01', 'ops_release_status', '待执行', 'pending', 1, 'default', '0', NULL, '000000', 0),
('dict_ops_rel_02', 'ops_release_status', '校验中', 'checking', 2, 'processing', '0', NULL, '000000', 0),
('dict_ops_rel_03', 'ops_release_status', '待审批', 'waiting_approval', 3, 'warning', '0', NULL, '000000', 0),
('dict_ops_rel_04', 'ops_release_status', '执行中', 'running', 4, 'processing', '0', NULL, '000000', 0),
('dict_ops_rel_05', 'ops_release_status', '成功', 'success', 5, 'success', '0', NULL, '000000', 0),
('dict_ops_rel_06', 'ops_release_status', '失败', 'failed', 6, 'error', '0', NULL, '000000', 0),
('dict_ops_rel_07', 'ops_release_status', '回滚中', 'rolling_back', 7, 'warning', '0', NULL, '000000', 0),
('dict_ops_rel_08', 'ops_release_status', '已回滚', 'rolled_back', 8, 'default', '0', NULL, '000000', 0),
('dict_ops_rel_09', 'ops_release_status', '已取消', 'cancelled', 9, 'default', '0', NULL, '000000', 0);
