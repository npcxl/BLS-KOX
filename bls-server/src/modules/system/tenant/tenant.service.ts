...
export class TenantService {
  ...
  listPublic() {
    return this.repository.listEnabledOptions();
  }
  ...
  async add(input: CreateTenantInput): Promise<any> {
  ...
  async edit(input: UpdateTenantInput): Promise<void> {
    ...
    if (input.tenantId === '000000') throw new ValidationError('平台租户不允许修改');
    await this.repository.update(input);
  }

  async remove(ids: string[]): Promise<void> {
    if (ids.length === 0) throw new ValidationError('请选择要删除的数据');
    if (ids.includes('000000')) throw new ValidationError('平台租户不允许删除');
    await this.repository.remove(ids);
  }

  async changeStatus(tenantId: string, status: '0' | '1'): Promise<void> {
    if (tenantId === '000000') throw new ValidationError('平台租户不允许禁用');
