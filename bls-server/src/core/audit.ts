...
import { auditWriteFailuresTotal } from '../observability/metrics';
 ...
 *
 * 审计失败**绝不**抛出到业务链路，但必须：
 *   - 递增 `bls_kox_audit_write_failures_total{type}`
 *   - 输出结构化错误日志（event: audit_write_failed）
 * 便于监控告警与事后追溯。
 */
export async function safeAuditWrite(type: string, fn: () => Promise<void>): Promise<void> {
  try {
    await fn();
  } catch (error) {
    auditWriteFailuresTotal.inc({ type });
...
export async function writeOperationLog(input: any): Promise<void> {
  await safeAuditWrite('operation', async () => {
    await execute(
...
export async function writeUploadAudit(input: any): Promise<void> {
  await safeAuditWrite('upload', async () => {
...
export async function writeLoginLog(input: LoginLogInput): Promise<void> {
  await safeAuditWrite('login', async () => {
    await execute(
